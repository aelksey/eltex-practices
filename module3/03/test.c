...
void test_file_b(void){
    int id;
    const char *filename = "teststorage.bin";
    book_t a;
    book_t b;
    a.size = 0;
    contact_t c1 = init_c(1,2,"Peter","Griffin");
    ...
    if(write_book(&a,filename) == -1)perror("Error writing to file");
    if(read_book(&b,filename,&id) == -1)perror("Error reading from file");
    
    CU_ASSERT_EQUAL(b.size,2);
    CU_ASSERT_STRING_EQUAL(b.contacts[0].name.firstname,"Peter");
    CU_ASSERT_STRING_EQUAL(b.contacts[0].name.lastname,"Griffin");

    CU_ASSERT_STRING_EQUAL(b.contacts[1].name.firstname,"Lois");
    CU_ASSERT_STRING_EQUAL(b.contacts[1].name.lastname,"Griffin");
}
...
int main(){
    ...
    CU_add_test(pSuite,"test : book_t: file i/o",test_file_b);
